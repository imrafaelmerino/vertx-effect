...
final class ParallelMap<O> extends vertx.effect.exp.Map<O> {
    ...
    ParallelMap(final Map<String, Val<? extends O>> bindings) {
     ...
     */
    public ParallelMap<O> set(final String key,
                              final Val<? extends O> exp
                             ) {
        return new ParallelMap<>(bindings.put(requireNonNull(key),
                                              requireNonNull(exp)
                                             ));
    }
    ...
    public Val<Map<String, O>> retry(final int attempts) {
        ...
        return new ParallelMap<>(bindings.mapValues(it -> it.retry(attempts)));
    }
    ...
    public Val<Map<String, O>> retryIf(final Predicate<Throwable> predicate,
        ...
        return new ParallelMap<>(bindings.mapValues(it -> it.retryIf(predicate,
                                                                     attempts
                                                                    ))
        );
