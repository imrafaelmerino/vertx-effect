...
final class SequentialMap<O> extends vertx.effect.exp.Map<O> {
    ...
    SequentialMap(final Map<String, Val<? extends O>> bindings) {
     ...
     */
    public vertx.effect.exp.Map<O> set(final String key,
                                       final Val<? extends O> exp
                                      ) {
        return new SequentialMap<>(bindings.put(requireNonNull(key),
                                                requireNonNull(exp)
                                               ));
    }
    ...
    public Val<Map<String, O>> retry(final int attempts) {
        ...
        return new SequentialMap<>(bindings.mapValues(it -> it.retry(attempts)));
    }
    ...
    public Val<Map<String, O>> retryIf(final Predicate<Throwable> predicate,
        ...
        return new SequentialMap<>(bindings.mapValues(it -> it.retryIf(predicate,
                                                                       attempts
                                                                      ))
        );
